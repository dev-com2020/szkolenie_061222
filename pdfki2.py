...
class StructuredPDF(fpdf.FPDF):
    ...
    def footer(self):
        ...
        # Numer strony. Zwróć uwagę na zastępowany człon {nb}.
        # Podwójny nawias klamrowy jest zastępowany przez jeden taki nawias.
        page_number = 'Strona {number}/{{nb}}'.format(number=self.page_no())
    ...
    def get_full_line(self, head, tail, fill):
        '''
        Zwraca wiersz spisu treści uzupełniony odpowiednią liczbą
...
def main():
    ...
    for index in range(1, num_chapters):
        chapter_title = 'Rozdzial {}'.format(index)
